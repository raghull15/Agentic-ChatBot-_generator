    ...
    return (
        <div className="min-h-screen bg-[var(--bg-primary)]">
            ...
            <div className="max-w-6xl mx-auto px-4 py-8 sm:py-12">
                ...
                {/* Plans Tab with Horizontal Scroll */}
                {activeTab === 'plans' && (
                    <div className="relative">
                        ...
                        {plans.length > 2 && (
                            <>
                                <button
                                    ...
                                    className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-full flex items-center justify-center hover:bg-[var(--bg-tertiary)] transition-colors shadow-lg"
                                ...
                                <button
                                    ...
                                    className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-full flex items-center justify-center hover:bg-[var(--bg-tertiary)] transition-colors shadow-lg"
                        ...
                        >
                            {plans.map((plan, idx) => (
                                <div
                                    key={plan.id}
                                    className={`card p-6 hover:scale-105 transition-all flex-shrink-0 w-80 ${idx === 1 ? 'border-2 border-[var(--primary)] shadow-xl' : ''
                                        }`}
                                    ...
                                    {usage.length === 0 ? (
                                        ...
                                        usage.map((log, idx) => (
                                            <tr key={log.id || idx} className="border-b border-[var(--border-color)] hover:bg-[var(--bg-tertiary)] transition-colors">
                                                <td className="p-4 text-[var(--text-primary)]">{log.chatbot_id || '—'}</td>
                                                <td className="p-4 text-center text-[var(--text-primary)]">{log.total_tokens?.toLocaleString()}</td>
                                                <td className="p-4 text-center font-semibold text-[var(--text-primary)]">{log.credits_used?.toFixed(2)}</td>
                                                <td className="p-4 text-right text-[var(--text-muted)]">{formatDate(log.created_at)}</td>
                                            </tr>
                                        ))
                                    )}
                                </tbody>
